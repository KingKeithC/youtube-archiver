...
# In our case, we intend to expand the remaining arguments, so SC2048 and SC2086 can be skipped for the next command.
# shellcheck disable=SC2048,SC2086
python -m youtube_dl \
 --config-location "$YTAR_YTDL_CONFIG_PATH" \
 --download-archive "$YTAR_ARCHIVE_PATH/archivefile" \
 --output "$YTAR_DOWNLOADS_PATH/%(uploader)s/playlists/%(playlist_title)s/%(playlist_index)s_%(title)s.%(ext)s" \
